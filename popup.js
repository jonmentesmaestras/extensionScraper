...
async function d() {
  null !== i.url.toLowerCase().match(/\/\/[a-z]+\.linkedin\.com/)
    ...
    : (I(),
      ...
      (function () {
        var remaining = 30;
        $("#waitHeader").text("Scrolling page... " + remaining + "s remaining");
        var ci = setInterval(function () {
          remaining--;
          $("#waitHeader").text(
            "Scrolling page... " + remaining + "s remaining",
          );
          remaining <= 0 && clearInterval(ci);
        }, 1e3);
        chrome.tabs.sendMessage(i.id, { action: "autoScroll" }, function () {
          R();
        });
      })());
}
...
function x(e, t) {
  ...
  ($("#wrongTable").show(),
    ...
    chrome.tabs.sendMessage(i.id, { action: "getTableData" }, function (e) {
      if (e) {
        if (e.error) return p("Something went wrong!", "noResponseErr", !0);
        e.tableId == s.tableId &&
          (e.failedToProcess
            ? (p(
          ...
          $("#csv")
            ...
            .click(function () {
              ...
              function getUrlParameter(name, searchStr) {
                ...
                var results = regex.exec(searchStr || i.url);
                return results === null
                  ? ""
                  : decodeURIComponent(results[1].replace(/\+/g, " "));
              }
              let currentKeyword = getUrlParameter("q");
              if (!currentKeyword) {
                currentKeyword = n;
              }
              const now = new Date();
              ...
              let newFilename = currentKeyword + "_" + datetimeStr + ".csv";
              saveAs(
              ...
              chrome.runtime.sendMessage(
                "pcjcfelpdpngehinnoghedlgankmbdpb",
                { action: "csvDownloaded", filename: newFilename },
                async function (response) {
                  console.log(
                    "Mensaje enviado a pcjcfelpdpngehinnoghedlgankmbdpb",
                    response,
                  );
                  try {
                    const sheetUrl =
                      "https://docs.google.com/spreadsheets/d/e/2PACX-1vRYJBwfDJPxNqYv3Z5XaSXjcurgtkoLJ5e1dzRyJmuC2kQpjUbY27aA3v0Im78qALndoUdUbJTgxc3c/pub?output=csv";
                    ...
                    const keywords = result.data
                      .filter((row) => {
                        ...
                        if (!kw) return false;
                        const trackLower = String(row.track || "")
                          .toLowerCase()
                          .trim();
                        const dateStr = String(row.scraped_date || "").trim();
                        return trackLower !== "done" && dateStr === "";
                      })
                      .map((row) => String(row.keyword).trim());
                    let currentIndex =
                      parseInt(localStorage.getItem("fbAdsKeywordIndex")) || 0;
                    let processedCount =
                      parseInt(
                        localStorage.getItem("fbAdsKeywordsProcessed"),
                      ) || 0;
                    let pauseUntil =
                      parseInt(localStorage.getItem("fbAdsPauseUntil")) || 0;
                    if (Date.now() < pauseUntil) {
                      const minsLeft = Math.ceil(
                        (pauseUntil - Date.now()) / 60000,
                      );
                      console.log(
                        `Rate limit pause active. Waiting ~${minsLeft} more minutes.`,
                      );
                      alert(
                        `Rate limit pause is active to prevent blocking.\nWaiting ${minsLeft} minutes before resuming.`,
                      );
                      return;
                    }
                    if (currentIndex < keywords.length) {
                      ...
                      localStorage.setItem(
                        "fbAdsKeywordsProcessed",
                        processedCount,
                      );
                      if (processedCount > 0 && processedCount % 40 === 0) {
                        console.log("40 keywords scraped. Triggering pause...");
                        localStorage.setItem(
                          "fbAdsPauseUntil",
                          Date.now() + 15 * 60 * 1000,
                        );
                        chrome.runtime.sendMessage(
                          { action: "setRateLimitAlarm", minutes: 15 },
                          () => {
                            window.close();
                          },
                        );
                      ...
                      console.log(
                        `Auto-navigating to keyword (${processedCount} processed in this burst):`,
                        currentKeyword,
                      );
                      const nextUrl =
                        "https://www.facebook.com/ads/library/?active_status=active&ad_type=all&country=BR&is_targeted_country=false&media_type=all&q=" +
                        encodeURIComponent(currentKeyword) +
                        "&search_type=keyword_unordered&sort_data[mode]=total_impressions&sort_data[direction]=desc";
                      localStorage.setItem(
                        "fbAdsKeywordIndex",
                        currentIndex + 1,
                      );
                      ...
                      console.log(
                        "Finished looping through all un-scraped keywords.",
                      );
                      localStorage.setItem("fbAdsKeywordIndex", 0);
                      localStorage.setItem("fbAdsKeywordsProcessed", 0);
                    }
                  } catch (err) {
                    console.error(
                      "Error fetching keywords from Google Sheets:",
                      err,
                    );
                  }
                },
